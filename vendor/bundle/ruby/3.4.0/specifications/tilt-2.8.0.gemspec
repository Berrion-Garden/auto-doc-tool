# -*- encoding: utf-8 -*-
# stub: tilt 2.8.0 ruby lib

Gem::Specification.new do |s|
  s.name = "tilt".freeze
  s.version = "2.8.0".freeze

  s.required_rubygems_version = Gem::Requirement.new(">= 0".freeze) if s.respond_to? :required_rubygems_version=
  s.metadata = { "bug_tracker_uri" => "https://github.com/jeremyevans/tilt/issues", "changelog_uri" => "https://github.com/jeremyevans/tilt/blob/master/CHANGELOG.md", "documentation_uri" => "https://tilt.jeremyevans.net", "mailing_list_uri" => "https://github.com/jeremyevans/tilt/discussions", "source_code_uri" => "https://github.com/jeremyevans/tilt" } if s.respond_to? :metadata=
  s.require_paths = ["lib".freeze]
  s.authors = ["Ryan Tomayko".freeze, "Magnus Holm".freeze, "Jeremy Evans".freeze]
  s.date = "1980-01-02"
  s.description = "Generic interface to multiple Ruby template engines".freeze
  s.email = "code@jeremyevans.net".freeze
  s.executables = ["tilt".freeze]
  s.files = ["bin/tilt".freeze]
  s.homepage = "https://github.com/jeremyevans/tilt".freeze
  s.licenses = ["MIT".freeze]
  s.rdoc_options = ["--line-numbers".freeze, "--inline-source".freeze, "--title".freeze, "Tilt".freeze, "--main".freeze, "Tilt".freeze]
  s.required_ruby_version = Gem::Requirement.new(">= 2.0".freeze)
  s.rubygems_version = "4.0.10".freeze
  s.summary = "Generic interface to multiple Ruby template engines".freeze

  s.installed_by_version = "3.6.9".freeze
end
